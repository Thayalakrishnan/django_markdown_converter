...
ORDERED_LIST_PATTERN = (r'(?:^ *\d+\. +.*$)+', re.MULTILINE | re.DOTALL)
UNORDERED_LIST_PATTERN = (r'(?:^ *- +.*$)+', re.MULTILINE | re.DOTALL)

BLOCKQUOTE_PATTERN = (r'(?:^> *?.*?$\n)+(?:\{.*?\})?', re.MULTILINE)
BLOCKQUOTE_PATTERN = (r'(?:^>.*$)+', re.MULTILINE | re.DOTALL)

ADMONITION_PATTERN = (r'(?:^!!!.*$)', re.MULTILINE | re.DOTALL)
