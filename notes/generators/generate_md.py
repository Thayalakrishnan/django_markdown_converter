...
def generate_sub_content(state:State=None, is_indented=False):
    """
...
def generate_meta(state:State=None):
    ...
    """
    pairs = [f"{fake.word()}: {LAMGEN_FAKE_WORDS()}" for _ in range(random.randint(1,5))]
...
def return_markdown_blocks_list(state:State=None, generator_funcs:Callable=lambda: None, num_blocks:int=1) -> list:
    return [random.choice(generator_funcs)(state) for _ in range(num_blocks)]
